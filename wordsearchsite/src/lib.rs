...
impl Wordsearch {
    pub fn init(path_to_wordsearch: &str) -> Self {
        ...
        for line in reader.lines() {
            ...
            for letter in line.chars() {
                for lower in letter.to_lowercase() {
                    wsearch.push(lower)
                }
            }
        }
    ...
    pub fn np_init(path_to_wordsearch: &str) -> Self {
        let mut wsearch: Vec<char> = Vec::new();
        let mut column: usize = 0;
...
impl WordBlob {
    pub fn alloc(path_to_wordsearch: &str, path_to_dictionary: &str, np: bool) -> Self {
        Self {
            wordsearch: match np {
                false => Wordsearch::init(path_to_wordsearch),
                true => Wordsearch::np_init(path_to_wordsearch)
            },
