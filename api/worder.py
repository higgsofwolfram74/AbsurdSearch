...
class InvalidWriteAttempt(Exception):
    #handle when we write where we should not
    pass

class MalformedWordsearch(Exception):
    #handle when we start writing when we shouldn't have
    pass
...
class Mutate:
    ...
    def go(self, width: int, length: int) -> bool:
        ...
        if self.temp_ws_index[0] < 0 or self.temp_ws_index[1] < 0:
            ...
            self.temp_ws_index = (self.temp_ws_index[0] + 1, self.temp_ws_index - width)
...
class WordsearchGenerator(WordPlacer):
    ...
    def __init__(self, wordlist: Words) -> None:
        self.words = wordlist
...
#reduce boilerplate for creating wordsearch
def create(wordlist: List[str]) -> WordsearchGenerator:
    wordsearch = WordsearchGenerator(Words(wordlist))
